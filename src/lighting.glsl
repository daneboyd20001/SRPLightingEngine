...
{
    float eps = .01;
    vec3 ddx = GetGradient(p + vec3(eps, 0, 0)) - GetGradient(p - vec3(eps, 0, 0)) / (2 * eps);
    vec3 ddy = GetGradient(p + vec3(0, eps, 0)) - GetGradient(p - vec3(0, eps, 0)) / (2 * eps);
    vec3 ddz = GetGradient(p + vec3(0, 0, eps)) - GetGradient(p - vec3(0, 0, eps)) / (2 * eps);
...
vec3 GetSurfaceEmission(vec3 p)
{
    ...
    vec3 col1 = vec3(1, .45, .1) * sin(time / 11) + vec3(.1, .85, 1) * cos(time / 11);
    vec3 col2 = vec3(.1, .1, .9) * sin(time / 23) + vec3(.76, .1, 1) * cos(time / 23);
...
vec3 Lighting(rayHit hit) {
  vec3 lightSource = normalize(vec3(sin(time), cos(time), 0.0));
    ...
    switch (activeLighting) {
      case 0: {
                finalLighting *= lampStrength/((hitBuffer.dist+1)*(hitBuffer.dist+1));
                break;
              }
        case 1: {
            finalLighting *= lampStrength/((dist+1)*(dist+1));

            float lambert = dot(hitBuffer.normal, -hitBuffer.rayDir);
            float rimLighting = 1.0 - lambert;
            float fresnel = pow(1.0 - abs(dot(hitBuffer.normal, hitBuffer.rayDir)), 4.0);
        ...
        case 2: {
            ...
            float lambert = max(dot(hitBuffer.normal, -hitBuffer.rayDir), 0.0);
 
            finalLighting *= lambert * exp(-hitBuffer.dist/lampStrength);
            break;
        }
        case 3: {
            finalLighting *= exp(-hitBuffer.dist/lampStrength);

            break;
        }
        case 4:{
            float t = exp(-hitBuffer.dist/lampStrength);
